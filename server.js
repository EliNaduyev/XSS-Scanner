    ...
    for (script in MALICIOUS_SCRIPT){
      ...
      for (i in formsArray){
        try {
          ...
          let inputsArray = await formsArray[i].$$eval('input[type="text"],input[type="search"],input:not([type]),textarea', inputs => inputs.map(input => input.id ? '#'+input.id : '.' + input.className));
...
const check_url = (url,script)=> {
  ...
  if (temp[1]){
    newUrl= temp[0] + '?';
    for (let k in tmp){
      if(key[0]===k){ newUrl= newUrl + k + '=' + script; }
      else { newUrl= newUrl+ '&'+ k + '=' + script; }
    }
  }
  return newUrl
}
